...
static void	scommand_tail_loop(const char *command, int fd, int last_line, size_t first_file)
{
	char	*file_name;
	size_t	count_args;
	int		input_fd;
	...
	while (i < count_args)
	{
		...
		write_fetch_tail(input_fd, last_line, fd, false);
		free(file_name);
		close(input_fd);
		i += 1;
	}
...
void	command_tail(const char *command, int fd)
{
	...
	first_file = 1;
	count_line = 10;
	if (is_valid_command(command, "tail", 0) == false)
		return ;
	...
	if (is_int == false)
	{
		write(fd, "tail : invalid number of lines: `", 33);
		word_outin_quote(parse_words(command, -2, 2, ITH_ARG) + command, fd);
		write(fd, "`\n", 2);
	}
	else
		scommand_tail_loop(command, fd, count_line, first_file);
}

static void	scommand_head_loop(const char *command, int fd, int first_line, size_t first_file)
{
	char	*file_name;
	size_t	count_args;
	int		input_fd;
