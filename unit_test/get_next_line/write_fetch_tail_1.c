...
int	main(void)
{
	...
	char	*output_dir = "unit_test/get_next_line/output/write_fetch_tail_1.txt";
	int				input_file;
	int				output_file;
	char			*output_gnl;
	...
	while (i < max_score)
	{
		...
		write_fetch_tail(input_file, 1, output_file, true);
		close(input_file);
		i += 1;
	}
	close(output_file);
	...
	while (i < max_score)
	{
		...
		else
		{
			...
			write(1, "\t", 1);
			ft_putnbr_fd(f_strlen(output_gnl), 1, "0123456789", 1);
			write(1, "\t", 1);
			ft_putnbr_fd(f_strlen(arr[i].first_line), 1, "0123456789", 1);
			write(1, "\n", 1);
		}
		free(output_gnl);
		i += 1;
...
/*
valgrind --leak-check=full --show-leak-kinds=all ./unit_test/out/get_next_line/write_fetch_tail_1.out
